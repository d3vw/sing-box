...
func TestPortalAdminPageRendersOwnCustomOutbound(t *testing.T) {
	dir := t.TempDir()
	manager := newTestPortalManager()
	h := &portalOutbound{manager: manager, memberOutboundConfigDirectory: dir}

	page := h.renderAll(testAdminInboundTag, testAdminUserName)

	if !strings.Contains(page, "Custom Outbound") {
		t.Fatalf("expected admin's own card to render Custom Outbound section, got:\n%s", page)
	}
}

func TestPortalAdminCanSaveOwnCustomOutbound(t *testing.T) {
	dir := t.TempDir()
	manager := newTestPortalManager()
	...
	response := servePortalRequestTestable(t, h, testAdminInboundTag, testAdminUserName, request)

	if response.StatusCode != http.StatusSeeOther {
		t.Fatalf("expected admin to be able to save their own outbound, got %s", response.Status)
	}

	quotaPath := filepath.Join(dir, memberSSConfigName(testAdminInboundTag, testAdminUserName))
	if _, err := os.Stat(quotaPath); err != nil {
		t.Fatalf("expected .quota file to exist for admin's own outbound: %v", err)
	}
}
...
func TestPortalMemberConnectivityFailureStillSavesWithWarning(t *testing.T) {
	dir := t.TempDir()
	manager := newTestPortalManager()
	...
	// Connectivity failure is advisory: save proceeds, response carries the warning.
	if response.StatusCode != http.StatusOK {
		t.Fatalf("expected 200 with warning when connectivity fails, got %s", response.Status)
	}
	body := readResponseBody(t, response)
	if !strings.Contains(body, "urltest failed") || !strings.Contains(body, "connectivity check failed") {
		t.Fatalf("expected connectivity warning in response, got:\n%s", body)
	}

	// .quota file MUST exist despite the failed probe
	quotaPath := filepath.Join(dir, memberSSConfigName(testInboundTag, testUserName))
	if _, err := os.Stat(quotaPath); err != nil {
		t.Fatalf("expected .quota file to exist after advisory connectivity failure: %v", err)
	}

	// live outbound MUST be active
	if _, ok := h.portalOutbound.liveOutbounds.Load(liveOutboundKey(testInboundTag, testUserName)); !ok {
		t.Fatal("expected liveOutbounds to contain member outbound after advisory connectivity failure")
	}

	// route fragment MUST be written on first save
	fragPath := filepath.Join(dir, memberOutboundFragmentName(testInboundTag, testUserName))
	if _, err := os.Stat(fragPath); err != nil {
		t.Fatalf("expected route fragment to exist after advisory connectivity failure: %v", err)
	}
}
...
func TestSubscriptionURI(t *testing.T) {
	state := &UserState{
		Key:                UserKey{InboundTag: "ss-in", UserName: "Grey"},
		Method:             "2022-blake3-aes-128-gcm",
		ServerPassword:     "serverpsk",
		UserPassword:       "userpsk",
		SubscriptionServer: "example.com",
		SubscriptionPort:   8888,
	}
	uri, ok := state.SubscriptionURI()
	if !ok {
		t.Fatal("expected ok=true")
	}
	want := "ss://" + base64.RawURLEncoding.EncodeToString([]byte("2022-blake3-aes-128-gcm:serverpsk:userpsk")) + "@example.com:8888#Grey"
	if uri != want {
		t.Fatalf("unexpected uri: got %q want %q", uri, want)
	}
}

func TestSubscriptionURIMissingInfo(t *testing.T) {
	state := &UserState{Key: UserKey{InboundTag: "ss-in", UserName: "Grey"}}
	if _, ok := state.SubscriptionURI(); ok {
		t.Fatal("expected ok=false when no subscription info is configured")
	}
}

func TestPortalOwnPageRendersSubscription(t *testing.T) {
	manager := newTestPortalManager()
	state := manager.userState(testInboundTag, testUserName)
	state.Method = "2022-blake3-aes-128-gcm"
	state.ServerPassword = "serverpsk"
	state.UserPassword = "userpsk"
	state.SubscriptionServer = "example.com"
	state.SubscriptionPort = 8888

	h := &portalOutbound{manager: manager}
	response := servePortalRequest(t, h, testInboundTag, testUserName, "GET / HTTP/1.1\r\nHost: quota.local\r\n\r\n")
	body := readResponseBody(t, response)

	if !strings.Contains(body, "ss://") || !strings.Contains(body, "Subscription") {
		t.Fatalf("expected subscription section in own page, got:\n%s", body)
	}
}
...
func TestListenPacketWithLiveOutboundForwards(t *testing.T) {
	manager := newTestPortalManager()
	listenCalled := false
	mock := &mockDialOutbound{listenFunc: func(ctx context.Context, destination M.Socksaddr) (net.PacketConn, error) {
		listenCalled = true
		return nil, nil
	}}
	...
	_, err := h.ListenPacket(ctx, M.SocksaddrFrom(netip.MustParseAddr("8.8.8.8"), 53))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !listenCalled {
		t.Fatal("expected live outbound ListenPacket to be called")
	}
}

func TestListenPacketWithoutLiveOutboundErrors(t *testing.T) {
	h := &portalOutbound{manager: newTestPortalManager()}

	metadata := &adapter.InboundContext{Inbound: testInboundTag, User: testUserName}
	ctx := adapter.WithContext(context.Background(), metadata)

	if _, err := h.ListenPacket(ctx, M.SocksaddrFrom(netip.MustParseAddr("8.8.8.8"), 53)); err == nil {
		t.Fatal("expected error when no live outbound is configured for the user")
	}
}
...
func (h *portalOutboundTestable) handleSaveShadowsocksOutboundMock(conn net.Conn, req *http.Request, inboundTag, userName string, isAdmin bool) {
	...
	if _, testErr := h.portalOutbound.testMemberShadowsocksOutbound(inboundTag, form.Server, form.ServerPort, form.Method, form.Password, form.Mux, form.Padding); testErr != nil {
	...
	if connectivityWarning != "" {
		h.portalOutbound.writeHTML(conn, http.StatusOK, buildPage("Saved with warning", fmt.Sprintf(
			`<div class="card">Outbound saved and activated, but the connectivity check failed:<br><br>%s<br><br>If the server works for you, ignore this. Otherwise use <b>Test</b> to re-check. <a href="/" style="color:#93c5fd">Back</a></div>`,
...
type mockDialOutbound struct {
	...
	listenFunc func(ctx context.Context, destination M.Socksaddr) (net.PacketConn, error)
}
...
func (m *mockDialOutbound) ListenPacket(ctx context.Context, destination M.Socksaddr) (net.PacketConn, error) {
	if m.listenFunc != nil {
		return m.listenFunc(ctx, destination)
	}
	return nil, errors.New("mock udp")
}
