...
func (h *portalOutbound) serveHTTP(conn net.Conn, inboundTag, userName string) {
	...
	if isAdmin {
		...
		body = buildPage(inboundTag+" / "+userName, renderSnapshot(snapshot, false, formData, subscriptionURI))
	} else {
...
func (h *portalOutbound) renderAll(selfInboundTag, selfUserName string) string {
	...
	for _, s := range snapshots {
		...
		sb.WriteString(renderSnapshot(s, true, formData, subscriptionURI))
	}
	if sb.Len() == 0 {
...
func renderSubscription(uri string) string {
    ...
    <div class="subscription-section">
      %s
      <div class="subscription-hint">Scan with your client, or copy the link</div>
      <button type="button" class="copy-button" data-uri="%s" onclick="copySubscription(this)">Copy link</button>
    </div>
  </div>
<script>
