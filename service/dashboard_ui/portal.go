...
func (h *portalOutbound) buildHandler() http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Only plain GETs are static asset/page loads. Everything else —
		// POST (gRPC-Web), OPTIONS (CORS preflight), and WebSocket upgrades
		// (bidirectional gRPC streams) — must reach the API handler, or the
		// browser's CORS preflight silently gets the SPA's index.html back
		// with no Access-Control-Allow-* headers and blocks the real call.
		if r.Method == http.MethodGet && !httpguts.HeaderValuesContainsToken(r.Header.Values("Upgrade"), "websocket") {
